...
INSERT [dbo].[Bills] ([ID], [Name], [TableID], [Amount], [Discount], [Tax], [Status], [CheckoutDate], [Account]) VALUES (1, N'Hóa đơn thanh toán', 5, 150000, 0.05, 0, 1, CAST(N'2021-11-03' AS SmallDateTime), N'tdquy')
...
select Name, Amount, Discount, Tax, Status, CheckoutDate from Bills where CheckoutDate Between '2021-10-04'and '2021-11-05'

select A.Name As TenMonAn, A.Unit As DonVi, A.Price As Gia, Price * Quantity As TongTien 
from Food A, BillDetails B
Where A.ID = B.FoodID
...
--Thêm và cập nhật RoleAccount
CREATE PROC AddRoleAccount
	@ID int,
...
----------------------------------------
--Thủ tục lấy tất cả dữ liệu bảng Account
CREATE PROC [dbo].[Account_GetAll]
AS
	SELECT * FROM [Table]

----------------------------------------
--Thủ tục lấy tất cả dữ liệu bảng Bills
CREATE PROC [dbo].[Bills_GetAll]
AS
	SELECT * FROM Bills

----------------------------------------
--Thủ tục lấy tất cả dữ liệu bảng Table
